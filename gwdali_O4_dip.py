...
dipole_gal = SkyCoord(268*u.deg, 48*u.deg, frame='galactic')
...
ra = inj['injections_parameters']['ras']
dec = inj['injections_parameters']['decs']

phi = ra #coord_O5_2det.ra.value*np.pi/180
theta = np.pi/2-np.array(dec) #coord_O5_2det.dec.value*np.pi/180
...
m1d_g = inj['injections_parameters']['m1d']*(1+dipole)
m2d_g = inj['injections_parameters']['m2d']*(1+dipole)
dl_g = inj['injections_parameters']['dls']*(1+dipole)
...
for j in range(0, len(inj['injections_parameters']['m1d'])):
    ...
    injection_parameters = dict(m1=m1d_g[j], m2=m2d_g[j],
                            RA=inj['injections_parameters']['ras'][j], Dec=inj['injections_parameters']['decs'][j], psi=inj['injections_parameters']['psis'][j], 
                            DL=dl_g[j], iota=inj['injections_parameters']['incs'][j], t_gps=inj['injections_parameters']['geocent_time'][j], t_coal=0, phi_coal=0,
    ...
    np.savetxt('Fisher_sampler_O4_inc_dip/Fisher_samples_'+str(j)+'.dat', Samples, header='mass_1\tmass_2\tluminosity_distance\tiota\tra\tdec')
    np.savetxt('covariance_O4_inc_dip/cov_'+str(j)+'.txt', CovFish)
